...
const insertPerson = async function(nbPerson) {
    ...
    console.log("Insertion Personnes : Génération des personnes")
    ...
    if (nbPerson <= 20 ) {
        return {
            "status" : 409,
            "data" : "Vous devez inserer plus de 20 personnes car chaque personne peut suivre jusqu'à 20 personnes différentes",
            "time" : null   
        }
    }
    ...
    console.log("Insertion Personnes : Récupération de toutes les personnes en base de données SQL")
    ...
    console.log("Insertion Personnes : Génération des relations")
    ...
    return {
        "status" : 200,
        "data" : "Les "+nbPerson+" personnes ont été crées en base de données MySQL ainsi que leurs relations",
        "time" : {
...
const insertProducts = async function(nbProducts) {
    ...
    console.log("Insertion Produits : Récupération des produits")
    var queryReturn = await sequelize.query(sql).catch(function(error) {
        return {
            "status" : 409,
            "data" : error.parent.sqlMessage,
    ...
    console.log("Insertion Produits : Génération des produits")
    ...
    while ((batchSize * (batch+1)) < productsTab.length) {
        ...
        console.log("Insertion Produits : Batch n°"+batch+" : Insertion de "+batchSize+" produits")
        var start = +new Date();
        queryReturn = await sequelize.query(sql).catch(function(error) {
            return {
                "status" : 409,
    ...
    console.log("Insertion Produits : Récupération de tous les produits")
    queryReturn = await sequelize.query(sql).catch(function(error) {
        return {
            "status" : 409,
            "data" : error.parent.sqlMessage,
    ...
    console.log("Insertion Produits : Génération des relations entre produits et personnes")
    ...
    while ((batchSize * (batch+1)) < relationTab.length) {
        ...
        console.log("Insertion Produits : Batch n°"+batch+" : Insertion de "+batchSize+" relations")
        start = +new Date();
        queryReturn = await sequelize.query(sql).catch(function(error) {
            return {
                "status" : 409,
    ...
    console.log("Insertion Produits : Envoie du résultat")
    return {
        "status" : 201,
        "data" : "Les "+nbProducts+" ont été crées ainsi que leurs relations",
        "time" : {
